...
#define M 70005
using namespace std;
int tmp, n, m, sz, lans, root;
int v[M], dfn[M], rt[M], ls[M], rs[M];
struct node{int l, r, sum;}a[N];
vector<int> rec, t, p;
inline int read(){
...
void insert(int &k, int x, int val){
	...
	if(a[rt[k]].sum*.75>max(a[rt[ls[k]]].sum, a[rt[rs[k]]].sum))
		if(tmp){
...
int main(){
	...
	while(m--){
		...
		int x = read(), y = read();
		switch(ch[0]){
		case 'Q':printf("%d\n", lans = solve_query(x, y, read()));break;
		case 'M':modify(root, x, y);break;
		case 'I':tmp = 0;insert(root, x-1, y);if(tmp){tmp = 0;rebuild(root);}
		}
	}
	return 0;
}
...
#ifndef M
#define M 1048575 // (1<<20)-1
#endif
template <typename T, int max_size = M>
...
public:
	void push(int x){
		while(l != r && q[r] < x) (--r)&=M;
		if((r-l&M) < max_size) q[(++r)&=M] = x;
	}
	void pop(){q[(++l)&=M]=0;}
	int front(){return q[l+1&M];}
	Monotonous_Queue(){q = new T[M];}
	~Monotonous_Queue(){delete [] q;}
};
int a[N];

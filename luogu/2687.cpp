...
int main(int argc, char const *argv[]){
	...
	for(int i = n-1; i; i--){
		...
		for(int j = i+1; j <= n; j++)
			if(a[j] < a[i])
				if(b[i] < b[j]+1) c[i] = c[j], b[i] = b[j]+1, mp[a[j]] = 1;
				else if(b[i] == b[j]+1 && !mp[a[j]]) c[i] += c[j], mp[a[j]] = 1;
	}
	int mx = -1<<30;
	huge nm = 0;
	// for(int i = 1; i < n; i++) printf("%d ",b[i] ),c[i].print();
	mp.clear();
	for(int i = 1; i < n; i++){
		if(mp[a[i]]) continue;
		mp[a[i]] = 1;
		if(b[i] > mx) mx = b[i], nm = c[i];
		else if(b[i] == mx) nm += c[i];
	}
	printf("%d ", mx); nm.print();
	return 0;
}